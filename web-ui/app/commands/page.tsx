...
export default function CommandsPage() {
  ...
  const fetchGlobal = useCallback(async () => {
    setIsLoading(true)
    try {
      const res = await itemApi.list({ type: 'command', limit: 500 })
  ...
  const filteredItems = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      f = f.filter(i => i.name.toLowerCase().includes(q) || i.slug?.toLowerCase().includes(q) || i.description?.toLowerCase().includes(q))
    }
    return f
